                ...
                case (\HpsTransactionType::CAPTURE): // Portico CreditAddtoBatch \HpsTransactionType::CAPTURE
                    ...
                    try {
                        if (\HpsInputValidation::checkAmount($reportTxnDetail->settlementAmount) > \HpsInputValidation::checkAmount($requestedAmount)) {
                            ...
                            $this->log(\HpsInputValidation::checkAmount($requestedAmount),
                                       'HPS\Heartland\Model\Payment reverse Method Called: requestedAmount');
            ...
            if (empty($errorMsg)) {
                ...
                switch (get_class($response)) {
                    ...
                    case 'HpsAuthorization':
                        ...
                        $successMsg[]
                            = __("The " . $info->getCcType() . " ending in {$CcL4} which expires on: " . $this->getAdditionalData()['cc_exp_month'] . " \\ " . $this->getAdditionalData()['cc_exp_year'] . " was {$actionVerb}
                                           [${$requestedAmount}] successfully. Your approval code is " . $response->authorizationCode);
